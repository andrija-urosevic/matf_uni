...
def maximum(state, alpha=float('-inf'), beta=float('inf')):
    if end_state(state):
    ...
    for next_state in legal_sates(state):
        (next_state_value, _) = minimum(next_state, alpha, beta)
        ...
        if best_state_value > alpha:
            alpha = best_state_value

    return (best_state_value, best_state)

def minimum(state, alpha=float('-inf'), beta=float('inf')):
    if end_state(state):
        return (evaluation_function(state), state)
    ...
    best_state = None
    for next_state in legal_sates(state):
        (next_state_value, _) = maximum(next_state, alpha, beta)
        if best_state_value > next_state_value:
            best_state_value = next_state_value
            best_state = next_state

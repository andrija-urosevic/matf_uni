...
lemma stepenuj_proizvod:
  shows "(a \<otimes> b) \<triangle> c = a \<triangle> c \<otimes> b \<triangle> c"
proof (induction c)
  case nula
  then show ?case 
    by simp
next
  case (Sled c)
  then show ?case 
    by (metis pomnozi_asoc pomnozi_kom stepenuj.simps(2))
qed
...
next
  case (Sled b)
  then show ?case 
    by (auto simp add: stepenuj_na_zbir stepenuj_proizvod)
qed
